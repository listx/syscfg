...
main = do
	...
	let
		mountablePaths = filter (\p -> not $ elem p takenPaths)
			. map (\p -> "/mnt/u" ++ show p)
			$ [(0::Int)..]
...
chooseDev
	:: String
	-> [(String, (BlockDevice, FilePath))]
	-> ((BlockDevice, FilePath) -> IO ExitCode)
	-> IO ()
...
getDevices Opts{..} = do
	...
	let
		toMount = filter (\BlockDevice{..} -> mountPoint == Unmounted) devs''
...
getUSBMountPath BlockDevice{..} = case mountPoint of
	MPath str -> if take 6 str == "/mnt/u"
		&& (all (\c -> elem c ['0'..'9']) (drop 6 str))
		then str
		else ""
	_ -> ""
...
p_braces
	:: ParsecT BL.ByteString () Identity a
	-> ParsecT BL.ByteString () Identity a
...
configParser = do
	-- take care of leading whitespace/comments as defined by configDef
	p_whiteSpace
