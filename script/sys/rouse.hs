...
main = do
    ...
    let statuses = zip4 (map show [1..]) (map fst _NODES) (map snd _NODES) onlines
...
getNodeStatus (link, mac) = do
    (sin, sout, serr, p) <- spawnCmd ("ping -c 1 -W 1 " ++ link ++ " >/dev/null 2>&1") True
...
wakeUp node@(_, link, mac, _) = do
    putStrLn $ "Waking " ++ showNode node ++ "..."
    (sin, sout, serr, p) <- spawnCmd ("wol -i " ++ _LAN_IP ++ " " ++ mac) True
    -- compile with -threaded in order to make waitForProcess non-blocking
    a <- waitForProcess p
    return ()
