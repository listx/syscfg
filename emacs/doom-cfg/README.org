...
Set up the "description" buffer of Jujutsu, to make it similar to how we have it
set up for Git.
...
** Do not insert newline after inserting stored link

The upstream definition always adds a newline, which is terrible if you're just
trying to insert a link into the middle of a paragraph. So do not pass in the
newline.

#+name: no-newline-after-inserting-stored-link
#+begin_src elisp
...
Make Org use the IDs when running =org-insert-link=. See
https://emacs.stackexchange.com/questions/64222/insert-link-to-a-heading-with-id.
To see where the ID-to-file mapping (cache) is stored, see the variable
~org-id-locations-file~, whose value is read into the ~org-id-locations~ hash table.

#+name: org-id-link-to-org-use-id
#+begin_src elisp
