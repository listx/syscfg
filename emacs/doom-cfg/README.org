...
Rename some "special" buffers, such as =dashboard.org= and journal entries in the form =YYYY-MM-DD.org=.

For =dashboard.org=, we just rename it to =DASHBOARD= because it's that important.

For journal entries, we appen a =[...]= suffix to it, depending on the relative date of it. If the date in the filename matches today's date, we add a =[TODAY]= suffix. For days in the past and future, we add a =[-N]= or =[+N]= suffix where =N= denotes the number of days that it is away from today, with negative numbers denoting days in the past. We use =[YESTERDAY]= and =[TOMORROW]= as aliases for =[-1]= and =[+1]=, respectively.

Unfortunately, it appears that we cannot use the file name as a source of truth because it is =nil= when this function is executed (it would be nice to use the file name because it is a reliable, mostly-immmutable source of truth).

#+name: l/org-rename-buffer
#+begin_src emacs-lisp
