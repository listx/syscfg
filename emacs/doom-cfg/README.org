...
This is generally useful. Doom by default sets this with the =(format +onsave)=
setting which enables =format-all-mode= everywhere, but this is too simplistic
and not useful for us. For example, for the Git project, although it has a
=.clang-format= (https://clang.llvm.org/docs/ClangFormat.html) file checked in,
it only uses it as a reference and the rules there are not actually enforced for
existing code.

Here we turn on format-all-mode everywhere, and also disable it for certain
directories. The overall structure here is based on the example given in
=format-all-mode='s documentation.
