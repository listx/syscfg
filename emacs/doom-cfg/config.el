...
(map! :after org
      :map org-mode-map
      :localleader
      "e" #'l/org-export-as-markdown-to-clipboard
      ; Rebind "org-export-dispatch" to "E".
...
(after! org
  (defun l/org-export-as-markdown-to-clipboard ()
  ...
  (unwind-protect (with-current-buffer ...) (kill-buffer buffer))."
    ...
    (let* ((org-export-with-toc nil)
           ...
           (buffer (save-window-excursion
                     (org-export-to-buffer 'md "*Formatted Copy*" ASYNC SUBTREEP VISIBLE-ONLY BODY-ONLY))))
      (with-current-buffer buffer
        ;; Delete leading newline from org-export-to-buffer.
        (goto-line 1)
        (flush-lines "^$")
        (unwind-protect
          ; "Kill" locally ("copy") into emacs. The word "kill" here is
          ; unfortunate because it is overloaded with the "kill" in "kill-buffer"
          ; below. Anyway we also send the buffer to an external "copy" program.
          (progn
